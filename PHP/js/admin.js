$(document).ready(function () {
    $('#menu').on('click', '.requests', function () {
        ...
        $.ajax({
            ...
            var data_table = '';
            data_table +=
                '<table><tr><th>Database ID</th><th>IP Address</th><th>Actions</th></tr>';
            $.each(res, function (i, item) {
                data_table +=
                    "<tr class='request'><td>" + item.databaseID + '</td>';
                data_table += '<td>' + item.ipaddress + '</td>';
                data_table +=
                    '<td><button>Remove</button> <button>Make Next</button> <button>Block IP</button></td></tr>';
            });
            data_table += '</table>';
